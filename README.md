...
├── README.md
├── project_plan.pdf
├── Rank Order Doctor Hospital Assignment.ipynb
├── src/
│   ├── assignment.py
│   ├── baseline.py
│   ├── input_data.py
...
- `src/validation.py`: Validates preference lists and hospital capacities.
- `src/input_data.py`: Parses manual rankings and generates reproducible random rankings.
- `src/assignment.py`: Creates capacity slots, builds the cost matrix, and solves the Hungarian assignment.
- `src/baseline.py`: Runs fixed-order or randomized greedy assignments.
- `src/metrics.py`: Evaluates assignments and summarizes repeated greedy trials.

## Environment

- Python 3.9 or newer
- No third-party Python packages are required for the current implementation.
- JupyterLab is required only for the interactive notebook.

The `requirements.txt` file documents that the core implementation has no external package dependencies.

## Run the Interactive Notebook

Start JupyterLab from the repository's parent course environment, then open:

```text
Rank Order Doctor Hospital Assignment.ipynb
```

Run the notebook cell and enter:
...
From the repository root, run the small reproducible example:

```bash
python -m examples.run_demo
```

Run the larger example with 20 doctors, 6 hospitals, and 1,000 randomized greedy trials:

```bash
python -m examples.large_example
```

Run the terminal version of the interactive demonstration:
...
- Valid and invalid preference input
- Insufficient hospital capacity
- Duplicate hospitals in a ranking
- Capacity compliance
- Complete assignment of all doctors
- Reproducibility of randomized greedy trials
- Agreement between the Hungarian result and brute-force optimal costs on small random matrices
...
```text
Hungarian total rank cost: 23
Hungarian average rank: 1.15
Hungarian first-choice rate: 85%
Randomized greedy mean total rank cost: 25.635
Randomized greedy best total rank cost: 23
Randomized greedy worst total rank cost: 33
```
